...
const cars = ref<any>([]); // Все автомобили
const filteredCars = ref<any>([]); // Для хранения отфильтрованных машин
const itemsPerPage = ref<any>(6); // Количество элементов для подгрузки за раз
const brands = ref<any[]>([]); // Список брендов
const selectedBrand = ref<any>(null); // Выбранный бренд
const popularCars = ref<any>([]); // Популярные автомобили
const promotionalCars = ref<any>([]); // Промо-автомобили
...
const allItemsLoaded = ref<boolean>(false); // Индикатор, что все элементы загружены
...
// Фильтрация по бренду
const filterByBrand = (brand: any) => {
  selectedBrand.value = brand; // Сохраняем текущий бренд

  // Фильтруем по бренду или показываем все автомобили
  if (brand) {
    filteredCars.value = cars.value
      .filter((car: any) => car.brand === brand)
      .slice(0, itemsPerPage.value); // Начальная порция для выбранного бренда
  } else {
    filteredCars.value = cars.value.slice(0, itemsPerPage.value); // Все автомобили
  }

  allItemsLoaded.value = false; // Сбрасываем флаг завершения подгрузки
};

// Подгрузка дополнительных автомобилей
const loadMoreCars = () => {
